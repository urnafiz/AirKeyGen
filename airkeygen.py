...
class Companion:
    ...
    def __init__(self, interface, save_result=False, print_debug=False, bssid=''):
        ...
        self.sessions_dir = f'{user_home}/.AirKeyGen/sessions/'
        self.pixiewps_dir = f'{user_home}/.AirKeyGen/pixiewps/'
...
#            print("[!] Unexpected interference — kill NetworkManager/wpa_supplicant!")
    ...
    def __saveAttackResult(self, bssid, essid, wps_pin, wpa_psk):
        """Always-on: append every successful attack to ~/.AirKeyGen/results.json."""
        user_home = str(pathlib.Path.home())
        results_dir = pathlib.Path(user_home) / '.AirKeyGen'
    ...
    def single_connection(self, bssid=None, pin=None, pixiemode=False, pbc_mode=False, showpixiecmd=False,
        ...
        if self.connection_status.status == 'GOT_PSK':
            ...
            self.__saveAttackResult(bssid, self.connection_status.essid, pin, self.connection_status.wpa_psk)
            if self.save_result:
...
def show_saved_results():
    """List successful attack results from ~/.AirKeyGen/results.json, newest first, with optional deletion."""
    user_home = str(pathlib.Path.home())
    results_file = pathlib.Path(user_home) / '.AirKeyGen' / 'results.json'

    if not results_file.exists():
        print('[i] No saved results found ({})'.format(results_file))
        return

    try:
        entries = json.loads(results_file.read_text(encoding='utf-8'))
    except (json.JSONDecodeError, OSError) as e:
        print('[!] Failed to read results file: {}'.format(e))
        return

    if not entries:
        print('[i] No saved results found.')
        return

    # Newest first: reverse chronological by saved_at, fall back to list order
    entries_indexed = list(enumerate(entries))   # keep original indices for deletion
    entries_indexed.sort(key=lambda x: x[1].get('saved_at', ''), reverse=True)

    col_bssid  = max(len(e.get('bssid',  '')) for _, e in entries_indexed)
    col_essid  = max(len(e.get('essid',  '')) for _, e in entries_indexed)
    col_pin    = max(len(e.get('wps_pin','')) for _, e in entries_indexed)
    col_psk    = max(len(e.get('wpa_psk','')) for _, e in entries_indexed)
    col_bssid  = max(col_bssid, 17)
    col_essid  = max(col_essid,  8)
    col_pin    = max(col_pin,    7)
    col_psk    = max(col_psk,    7)
    ...
    print('[i] Results file: {}'.format(results_file))
    print()
    print(header)
    print('-' * len(header))

    display_entries = []
    for display_idx, (orig_idx, e) in enumerate(entries_indexed, start=1):
        bssid    = e.get('bssid',    '')
        essid    = e.get('essid',    '')
        wps_pin  = e.get('wps_pin',  '')
        wpa_psk  = e.get('wpa_psk',  '')
        ...
        display_entries.append((orig_idx, e))

    print()
    print('Enter a number to delete that entry, or press Enter to cancel:')
    try:
        choice = input('> ').strip()
    ...
    try:
        sel = int(choice)
        if sel < 1 or sel > len(display_entries):
            raise ValueError
    except ValueError:
        print('[!] Invalid selection. No changes made.')
        return

    orig_idx, e = display_entries[sel - 1]
    confirm = input('[?] Delete result for {} / {} saved at {}? [y/N] '.format(
        e.get('bssid', ''), e.get('essid', ''), e.get('saved_at', '')
    )).strip().lower()

    if confirm == 'y':
        try:
            all_entries = json.loads(results_file.read_text(encoding='utf-8'))
            del all_entries[orig_idx]
            results_file.write_text(json.dumps(all_entries, indent=2, ensure_ascii=False), encoding='utf-8')
            print('[+] Entry deleted.')
        except Exception as ex:
            print('[!] Failed to delete entry: {}'.format(ex))
    else:
        print('[i] Cancelled. No changes made.')


def show_saved_pins():
    """List saved pins from the pixiewps folder, newest first, with optional deletion."""
    user_home = str(pathlib.Path.home())
    pixiewps_dir = pathlib.Path(user_home) / '.AirKeyGen' / 'pixiewps'

    if not pixiewps_dir.exists():
        print('[i] No saved pins directory found ({})'.format(pixiewps_dir))
        return

    run_files = sorted(
        pixiewps_dir.glob('*.run'),
        key=lambda f: f.stat().st_mtime,
        reverse=True  # newest first
    )

    if not run_files:
        print('[i] No saved pins found in {}'.format(pixiewps_dir))
        return

    print('[i] Saved pins directory: {}'.format(pixiewps_dir))
    print()
    print('{:<4} {:<20} {:<12} {}'.format('#', 'BSSID', 'PIN', 'Saved at'))
    print('-' * 60)
    ...
    for f in run_files:
        try:
            ...
            pin = '<unreadable>'
        raw_bssid = f.stem  # e.g. 00904CC1AC21
        # Re-format as XX:XX:XX:XX:XX:XX
        bssid = ':'.join(raw_bssid[i:i+2] for i in range(0, 12, 2)).upper()
        mtime = datetime.fromtimestamp(f.stat().st_mtime).strftime('%Y-%m-%d %H:%M:%S')
        entries.append((bssid, pin, mtime, f))

    for idx, (bssid, pin, mtime, _) in enumerate(entries, start=1):
        print('{:<4} {:<20} {:<12} {}'.format(idx, bssid, pin, mtime))
    ...
    bssid, pin, mtime, fpath = entries[idx - 1]
    confirm = input('[?] Delete PIN for {} ({})? [y/N] '.format(bssid, pin)).strip().lower()
    if confirm == 'y':
        try:
            fpath.unlink()
            print('[+] Deleted PIN entry for {}'.format(bssid))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-P', '--saved-pins',
        action='store_true',
        help='Show saved pins from the pixiewps folder (newest first) with option to delete'
        )
    parser.add_argument(
        '-R', '--saved-results',
        action='store_true',
        help='Show successful attack results (BSSID, ESSID, PIN, PSK, time) newest first, with option to delete'
        )

    args = parser.parse_args()

    # --saved-pins and --saved-results don't need root or an interface
    if args.saved_pins:
        show_saved_pins()
        sys.exit(0)
    ...
    if not args.interface:
        parser.error('the following arguments are required: -i/--interface')
